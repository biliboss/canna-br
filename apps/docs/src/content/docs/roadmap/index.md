...
- **Loop de chat completo — PASS (end-to-end ao vivo).** Validado em chrome-devtools (apps/agent → apps/mcp via HTTP, chave OpenRouter): mensagem → Claude chama `get_member_quota` → dado real do Postgres → **o widget `member-quota-card` renderiza INLINE no chat preenchido com os números reais** (barra 7g/30g, Status ACTIVE), zero erro de console. O processo destravou 5 bugs reais que os testes in-memory não pegavam: transport StreamableHTTP stateless por-request, `_meta` slash-form `ui/resourceUri`, headers de auth `x-canna-*`, e — a última milha — os widgets liam `payload.type` em vez do `payload.method` do bridge JSON-RPC + não mandavam `notifications/initialized`. Todos corrigidos. O loop de chat completo funciona ponta a ponta.
...
**Done-when (v0.1 funcional): ATINGIDO** para o caminho operacional central. O loop de chat completo foi validado ao vivo (chrome-devtools): o usuário pergunta no chat, o agente chama o tool do domínio, e o widget renderiza inline preenchido com dado real do Postgres — zero erro de console. Restam itens *deferidos para além do v0.1*: promover o `member-lifecycle-board` a widget vivo (precisa de um tool `get_member_lifecycle` + read-model de enumeração) e substituir os headers `x-canna-*` de dev por OIDC real (Zitadel). O sistema é usável de ponta a ponta: associação consulta cota/membro/rastreabilidade via chat com widgets inline.
