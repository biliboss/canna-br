import {
  ...
  type EventStore as EmmettEventStore,
} from "@event-driven-io/emmett";

const isStreamVersionConflict = (e: unknown): boolean => {
  if (isExpectedVersionConflictError(e)) return true;
  if (e instanceof ExpectedVersionConflictError) return true;
  if (e instanceof ConcurrencyError) return true;
  if (e instanceof Error) {
    const msg = e.message.toLowerCase();
    if (
      msg.includes("expected version") ||
      msg.includes("version conflict") ||
      msg.includes("stream already exists") ||
      msg.includes("stream does not exist")
    ) {
      return true;
    }
    const code = (e as { code?: string }).code;
    if (code === "23505" /* unique_violation */) return true;
  }
  return false;
};
...
const fromEmmett = <T extends DomainEvent<string, unknown>>(stored: {
  type: string;
...
/**
 * Wrap any Emmett EventStore (in-memory or Postgres) behind the
 * CannaEventStore interface. Translates DomainEvent ↔ Emmett event shape and
 * normalizes optimistic-concurrency conflicts to `StreamVersionConflictError`.
 */
export const wrapEmmettStore = (
  ...
  async aggregateStream(streamId, options): Promise<AggregateResult<never>> {
    const result = await store.aggregateStream<unknown, StoredEvent>(streamId, {
